...
Before deploying, store sensitive credentials in Secret Manager. Two secrets come
directly from Terraform output; the rest must be provided manually.

```bash
export PROJECT_ID="your-project-id"
gcloud config set project ${PROJECT_ID}
cd ../audio_text_infrastructure   # Terraform must have been applied first
```

**Secrets sourced from Terraform (run after `terraform apply`):**

```bash
# Database password — generated by Terraform random_password
terraform output -raw database_password \
  | gcloud secrets create audio-text-db-password \
      --project="${PROJECT_ID}" --data-file=- 2>/dev/null \
  || terraform output -raw database_password \
       | gcloud secrets versions add audio-text-db-password \
           --project="${PROJECT_ID}" --data-file=-
```

> **Tip:** The one-liner above creates the secret if it doesn't exist, or adds a
> new version if it does. Re-run it whenever `terraform apply` regenerates the password.

**Secrets provided manually (your own credentials):**

```bash
# AWS S3 credentials
echo -n "your-aws-access-key" \
  | gcloud secrets create audio-text-aws-access-key \
      --project="${PROJECT_ID}" --data-file=- 2>/dev/null \
  || echo -n "your-aws-access-key" \
       | gcloud secrets versions add audio-text-aws-access-key \
           --project="${PROJECT_ID}" --data-file=-

echo -n "your-aws-secret-key" \
  | gcloud secrets create audio-text-aws-secret-key \
      --project="${PROJECT_ID}" --data-file=- 2>/dev/null \
  || echo -n "your-aws-secret-key" \
       | gcloud secrets versions add audio-text-aws-secret-key \
           --project="${PROJECT_ID}" --data-file=-

# SendGrid API key
echo -n "your-sendgrid-api-key" \
  | gcloud secrets create audio-text-sendgrid-api-key \
      --project="${PROJECT_ID}" --data-file=- 2>/dev/null \
  || echo -n "your-sendgrid-api-key" \
       | gcloud secrets versions add audio-text-sendgrid-api-key \
           --project="${PROJECT_ID}" --data-file=-
```
...
gcloud secrets list --project="${PROJECT_ID}" \
  --filter="name:(audio-text-db-password OR audio-text-aws-access-key OR audio-text-aws-secret-key OR audio-text-sendgrid-api-key)"

# View latest version of a specific secret
gcloud secrets versions access latest --secret="audio-text-db-password" --project="${PROJECT_ID}"
```
